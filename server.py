...
class Handler(BaseHTTPRequestHandler):
    ...
    def _static(self, p):
        # The established dashboard stays at the root. React is served under
        # `/app`; API routing remains above this method so finance data is
        # never treated as a static asset.
        rel = "index.html" if p in ("/", "") else p.lstrip("/")
        root = os.path.join(HERE, "web")
        path = os.path.normpath(os.path.join(root, rel))
        if not path.startswith(root):
            return self._json({"error": "not found"}, 404)
        # BrowserRouter routes below `/app` are client-side. Requests there
        # receive its shell, while missing ordinary files remain 404s.
        if not os.path.isfile(path):
            if rel.startswith("app/") and "." not in os.path.basename(rel):
                path = os.path.join(root, "app", "index.html")
            elif "." in os.path.basename(rel):
                return self._json({"error": "not found"}, 404)
            else:
                return self._json({"error": "not found"}, 404)
        ctype = ("text/html" if path.endswith(".html") else
                 ...
                 "application/json" if path.endswith(".webmanifest") else "application/octet-stream")
