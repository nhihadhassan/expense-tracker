...
class Handler(BaseHTTPRequestHandler):
    ...
    def _static(self, p):
        # React's production bundle is generated by `npm run build`. Keep API
        # routing above this method so financial data is never a static asset.
        rel = "index.html" if p in ("/", "") else p.lstrip("/")
        root = os.path.join(HERE, "web-react")
        path = os.path.normpath(os.path.join(root, rel))
        if not path.startswith(root):
            return self._json({"error": "not found"}, 404)
        # BrowserRouter routes are client-side. Requests for non-API routes
        # therefore receive the shell, while missing files still remain 404s.
